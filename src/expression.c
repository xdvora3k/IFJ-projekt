...
void precedencSA(string *input, tExpressionList *expL, tLinkedList *linkedL)
{
    ...
    do
    {
        ...
        case '>': //zamen <y
            ...
            {
                InsertList(rule, &justTestingList, linkedL);
                printList(&justTestingList, rule);
            }
...
void printList(tExpressionList *List, tExpressionRule rule)
{
    ...
    if (rule.leftOperand)
    {
        ...
        {
        printf("%s", List->first->first->leftOperand->text->str);
        }
    ...
    {
        if(List->first->next_node){
        printf("%s", List->first->next_node->first->operator->text->str);
        } else {
        printf("%s", List->first->first->operator->text->str);
        }
    }
    if (rule.rightOperand)
    {
        if(List->first->next_node){
        printf("%s", List->first->next_node->first->rightOperand->text->str);
        } else {
        printf("%s", List->first->first->rightOperand->text->str);
        }    }
    if (rule.placeHolder)
    {
        printf(" ListPlace: %s", List->first->first->placeHolder->text->str);
    }
    printf("*************************\n");
}
...
/*
void CreateEXPList(tExpressionList *L , tToken *leftToken, tToken *rightToken, int counter){
    //ExprLLInit(L);
    printf("INSERT\n");

    printf("left %s and %d\n", leftToken->text->str, L->first->data_type);

}*/

/*tExpressionRule *rule*/

/* _insert_token_to_node(rule->rightOperand);
    _insert_token_to_node(rule->operator);
    _insert_token_to_node(rule->leftOperand);*/

/*tExpressionList InsertList(tExpressionRule rule){
    printf("insert\n");
  ...
  //  L->first->data_type = IntType;
    ...
    tToken* a = _insert_token_to_node(rule.leftOperand);
    _insert_token_to_node(rule.rightOperand);
    _insert_token_to_node(rule.operator);
    printf("a is %s and %d\n", a->text->str, a->type);
    printf("pointer %p\n", L->first);
    ...
    ExprLLInsertExprToLastNode(L, rule.leftOperand, rule.rightOperand, rule.operator, rule.placeHolder, rule.typeOfRule);
    printf("++++insert LIST %s +++\n", L->first->first->leftOperand->text->str);
