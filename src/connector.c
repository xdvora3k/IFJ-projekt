...
void print_variable_declaration_Expression(tLinkedList *leftside, tExpressionList *rightside,
    ...
    if (rule->leftOperand->type == tId)
        rule->leftOperand->text->str = VarLLGetRealName(final_variables, rule->leftOperand->text->str, NULL,
                                                        func_variable_list);
    for (int i = 0; i < expressionNumber; i++) {
        if (rule->rightOperand->type == tId)
            rule->rightOperand->text->str = VarLLGetRealName(final_variables, rule->rightOperand->text->str, NULL,
                                                             func_variable_list);
    }
...
char *Calc_Int_Expression(tExpressionNode *Rules, tLinkedList *func_variable_list) {
    ...
    for (int i = 0; i < expLength; i++) {
        ...
        } else { opL = ChangeOperand(opL, "", rule->leftOperand->text->str, IntType, Frame_LF); }

        if (rule->rightOperand->type == tId) {
            opR = ChangeOperand(opR, rule->rightOperand->text->str, "", IntType, Frame_LF);
        } else { opR = ChangeOperand(opR, "", rule->rightOperand->text->str, IntType, Frame_LF); }
...
char *Calc_Float_Expression(tExpressionNode *Rules, tLinkedList *func_variable_list) {
    ...
    for (int i = 0; i < expLength; i++) {
        ...
        else { opL = ChangeOperand(opL, "", rule->leftOperand->text->str, Float64Type, Frame_LF); }
        if (rule->rightOperand->type == tId) {
            opR = ChangeOperand(opR, rule->rightOperand->text->str, "", Float64Type, Frame_LF);
        }
        else { opR = ChangeOperand(opR, "", rule->rightOperand->text->str, Float64Type, Frame_LF); }
...
char *Calc_String_Expression(tExpressionNode *Rules, tLinkedList *func_variable_list) {
    ...
    for (int i = 0; i < expLength; i++) {
        ...
        else { opL = ChangeOperand(opL, "", rule->leftOperand->text->str, StringType, Frame_LF); }
        if (rule->rightOperand->type == tId) {
            opR = ChangeOperand(opR, rule->rightOperand->text->str, "", StringType, Frame_LF);
        }
        else { opR = ChangeOperand(opR, "", rule->rightOperand->text->str, StringType, Frame_LF); }
...
void print_function_assigment(tLinkedList *leftside, char *funcName, tPassedSide *variables, tPassedSide *params,
    ...
    if (strcmp(funcName, "inputi") == 0) {
        ...
        tInstructionOperand *retI = CreateOperand("retval", "", IntType, Frame_LF);
        tInstructionOperand *retII = CreateOperand("", "0", IntType, Frame_NaN);
        tInstructionOperand *opParamI = CreateOperand(
                VarLLGetRealName(final_variables, params->first->value, NULL, func_variable_list), "", IntType,
                Frame_LF);
        tInstructionOperand *opParamII = CreateOperand(
                VarLLGetRealName(final_variables, params->first->nextItem->value, NULL, func_variable_list), "",
                IntType, Frame_LF);
        ...
        tInstructionOperand *opParamII = CreateOperand(
                VarLLGetRealName(final_variables, params->first->nextItem->value, NULL, func_variable_list), "",
                IntType, Frame_LF);
        tInstructionOperand *opParamIII = CreateOperand(
                VarLLGetRealName(final_variables, params->first->nextItem->nextItem->value, NULL, func_variable_list),
                "", IntType, Frame_LF);
        ...
        for (int i = 0; i < numberInputVariables; i++) {
            ...
            tInstructionOperand *varI = CreateOperand(
                    VarLLGetRealName(final_variables, InputVariable->value, NULL, func_variable_list), "",
                    InputVariable->data_type, Frame_LF);
            tInstructionOperand *parI = CreateOperand(
                    VarLLGetRealName(final_variables, InputParameter->value, NULL, NULL), "", InputParameter->data_type,
                    Frame_LF);
...
void print_for_begin(tExpressionNode *expList, tLinkedList *leftsideAssigment, tExpressionList *rightsideAssigment,
    ...
    printf("DEFVAR TF@boolTmp2\n");
    fflush(stdout);
    printf("LABEL %s", specBeginFor.str);
    fflush(stdout);
